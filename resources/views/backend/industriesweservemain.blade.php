...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <h2 class="mb-4">Industries We Serve Main</h2>
            ...
            <!-- Add/Edit Form -->
            <div class="card mb-4 {{ !$canAdd && !isset($editIndustry) ? 'opacity-50' : '' }}">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0" id="formTitle">
                        {{ isset($editIndustry) ? 'Edit Industry' : 'Add New Industry' }}
                        @if(!$canAdd && !isset($editIndustry))
                            <span class="badge bg-warning text-dark ms-2">Limit Reached (Max 1 Item)</span>
                        @endif
                    </h5>
                </div>
                <div class="card-body">
                    <form action="{{ isset($editIndustry) ? route('industriesweservemain.update', $editIndustry->id) : route('industriesweservemain.store') }}" 
                          method="POST" 
                          enctype="multipart/form-data">
                        @csrf
                        
                        <fieldset {{ !$canAdd && !isset($editIndustry) ? 'disabled' : '' }}>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="heading1" class="form-label">Industry Name *</label>
                                        <input type="text" 
                                               class="form-control @error('heading1') is-invalid @enderror" 
                                               id="heading1" 
                                               name="heading1" 
                                               value="{{ old('heading1', isset($editIndustry) ? $editIndustry->heading1 : '') }}"
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        @if(isset($editIndustry) && $editIndustry->image1)
                                            <div class="mt-2" id="currentImage">
                                                <img src="{{ asset('uploads/industries/' . $editIndustry->image1) }}" 
                                                     alt="Current Image" 
                            ...
                            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                @if(isset($editIndustry))
                                    <a href="{{ route('industriesweservemain') }}" class="btn btn-secondary me-md-2">
                                        Cancel Edit
                                    </a>
                        ...
                        @if(!$canAdd && !isset($editIndustry))
                            <div class="alert alert-warning mt-3 mb-0">
                                <i class="fas fa-info-circle"></i> Only one industry item is allowed. Please delete the existing item to add a new one.
                            </div>
                        @endif
                    </form>
                </div>
            ...
            <div class="card">
                <div class="card-header bg-dark text-white">
                    <h5 class="mb-0">All Industries We Serve</h5>
                </div>
                <div class="card-body">
                        ...
                        <table class="table table-hover table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($industry->image1)
                                            <img src="{{ asset('uploads/industries/' . $industry->image1) }}" 
                                                 alt="{{ $industry->heading1 }}" 
                                    ...
                                    <td>{{ $industry->heading1 ?: '-' }}</td>
                                    <td>
                                        <small>{{ $industry->created_at->format('d M Y') }}</small><br>
                                        <small class="text-muted">{{ $industry->created_at->format('h:i A') }}</small>
                                    </td>
                                    <td class="text-center">
                                        <a href="{{ route('industriesweservemain.edit', $industry->id) }}" 
                                           class="btn btn-sm btn-warning mb-1"
                                           title="Edit">
                                            <i class="fas fa-edit"></i> Edit
                                        </a>
                                        <form action="{{ route('industriesweservemain.delete', $industry->id) }}" 
                                              method="POST" 
                                              class="d-inline"
