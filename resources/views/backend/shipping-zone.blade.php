...
<div class="page-container">
    ...
    <div class="two-col">
        ...
        {{-- FORM CARD --}}
        <div class="page-card">
            <div class="{{ isset($editZone) ? 'card-header-warning' : 'card-header-gradient' }}">
            ...
            <div class="card-body">
                <form action="{{ isset($editZone) ? route('shipping.zone.update', $editZone->id) : route('shipping.zone.store') }}"
                    ...
                    <div class="form-group">
                        ...
                        <small style="font-size:10px; color:#6b7280; margin-top:3px; display:block;">Leave empty for "Rest of World"</small>
                    </div>
                    ...
                    <div class="btn-group-custom">
                        @if(isset($editZone))
                            <a href="{{ route('shipping.zone') }}" class="btn btn-secondary">
        ...
        <div class="page-card">
            <div class="card-header-dark">
                <div class="card-header-row">
                    <h2 class="card-header-title"><i class="fas fa-list"></i> All Zones</h2>
                    <span class="table-count">Total: {{ $zones->count() }}</span>
                </div>
            </div>
            <div class="card-body" style="padding:0;">
                @if($zones->isEmpty())
                    <div class="empty-state">
                        ...
                        <p>Koi shipping zone nahi mila. Upar se add karein!</p>
                    </div>
                @else
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="font-weight:700; color:#0a214f;">{{ $zone->name }}</td>
                                    <td>
                                        @if(!empty($zone->countries))
                                            <span style="font-size:10px; color:#6b7280;">
                                                {{ implode(', ', array_slice($zone->countries, 0, 3)) }}
                                                @if(count($zone->countries) > 3)
                                                    <span class="badge badge-inactive">+{{ count($zone->countries) - 3 }}</span>
                                                @endif
                                            </span>
                                        @else
                                            <span class="badge badge-info">All Countries</span>
                                        @endif
                                    </td>
                                    <td>
                                        <span class="badge badge-primary">{{ $zone->rates()->count() }} rates</span>
                                    </td>
                                    <td>
                                        @if($zone->is_active)
