...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .btn-dark { background: linear-gradient(135deg, #1f2937, #374151); color: white; box-shadow: 0 4px 12px rgba(31,41,55,0.3); }
    .btn-dark:hover { transform: translateY(-1px); color: white; }
    ...
    .logo-preview {
        max-width: 70px; max-height: 38px; object-fit: contain;
        background: #f9fafb; padding: 4px 6px; border-radius: 6px;
        border: 1px solid #e5e7eb;
    }

    .url-text { font-size: 10px; color: #6b7280; max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; display: block; }
    ...
    .existing-logo { margin-top: 8px; padding: 6px 8px; background: #f0f4f8; border-radius: 6px; display: inline-block; }
...
<div class="page-container">
    ...
    <div class="page-header">
        <h1 class="page-title">🏷️ Brands Section</h1>
        <p class="page-subtitle">Manage brands displayed on the homepage</p>
    </div>
    ...
    <div class="two-col">
        ...
        {{-- LEFT COLUMN: Section Settings + Add/Edit Brand --}}
        <div class="left-col">
                ...
                <div class="card-body">
                    <form action="{{ route('brand.section.save') }}" method="POST">
                        ...
                        <div class="form-check form-switch">
                            ...
                            <label class="form-check-label" for="sec_active">Show on Frontend</label>
                        </div>
            ...
            {{-- ADD / EDIT BRAND CARD --}}
            <div class="page-card">
                <div class="{{ isset($editBrand) ? 'card-header-warning' : 'card-header-gradient' }}">
                    <h2 class="card-header-title">
                        @if(isset($editBrand))
                            <i class="fas fa-pen"></i> Edit Brand #{{ $editBrand->id }}
                        @else
                ...
                <div class="card-body">
                    ...
                    @csrf
                        ...
                        <div class="form-group">
                            <label class="form-label">
                                Brand URL
                                <small>Optional — e.g. /collections?brand=bayer</small>
                            </label>
                            <input type="text" name="url" class="form-control"
                                value="{{ old('url', $editBrand->url ?? '') }}"
                                placeholder="/collections?brand=xyz">
                        </div>
                        ...
                        <div class="form-check form-switch">
                            ...
                            <label class="form-check-label" for="brand_active">Active (Show on frontend)</label>
                        </div>

                        {{-- Buttons --}}
                        <div class="btn-group-custom">
                            @if(isset($editBrand))
                                <a href="{{ route('brand.section') }}" class="btn btn-secondary">
        ...
        <div class="page-card">
            <div class="card-header-gradient">
                <div class="card-header-row">
                    <h2 class="card-header-title"><i class="fas fa-list"></i> All Brands</h2>
                    <span class="table-count">Total: {{ $brands->count() }}</span>
                </div>
            </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td style="text-align:center;">
                                    <span class="badge badge-id">#{{ $brand->id }}</span>
                                </td>
                                <td>
                                    @if($brand->image)
                                ...
                                <td>
                                    @if($brand->url)
                                        <span class="url-text" title="{{ $brand->url }}">{{ $brand->url }}</span>
                                    @else
                                        <span class="badge badge-secondary">—</span>
                                    @endif
                                </td>
                                <td style="text-align:center;">
                                    <span style="font-weight:700;color:#0a214f;font-size:12px;">{{ $brand->sort_order }}</span>
                                </td>
                                <td style="text-align:center;">
                                    @if($brand->is_active)
                                ...
                                <td style="text-align:center;">
                                    <div style="display:flex; gap:5px; align-items:center; justify-content:center;">
                                        ...
                                        <form action="{{ route('brand.delete', $brand->id) }}"
                                              ...
                                              onsubmit="return confirmDelete(event, '{{ addslashes($brand->alt_tag ?? 'Brand #' . $brand->id) }}')">
                                            @csrf
                            ...
                            <tr>
                                <td colspan="7">
                                    <div class="empty-state">
                                        ...
                                        <p>No brands yet. Add your first one!</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
