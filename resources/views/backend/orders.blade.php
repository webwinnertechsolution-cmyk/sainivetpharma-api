...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .stat-card {
        ...
        box-shadow: 0 4px 16px rgba(10,33,79,0.08); border: 1px solid #e5e7eb;
    ...
    .stat-icon.total { background: linear-gradient(135deg, #0a214f, #1872B5); }
    .stat-icon.pending { background: linear-gradient(135deg, #b45309, #f59e0b); }
    .stat-icon.paid { background: linear-gradient(135deg, #059669, #34d399); }
    .stat-value { font-family: 'Sora', sans-serif; font-size: 18px; font-weight: 800; color: #0a214f; line-height: 1.2; }
    ...
    .filter-bar { padding: 14px 16px; background: #f9fafb; border-bottom: 1px solid #e5e7eb; display: flex; gap: 10px; flex-wrap: wrap; align-items: center; }
    ...
    .badge-order-id { background: #e0e7ff; color: #3730a3; font-size: 11px; padding: 4px 10px; }
    ...
    .pagination-wrap nav ul { display: flex; gap: 4px; list-style: none; padding: 0; margin: 0; font-size: 12px; }
    ...
    <div class="page-card">
        <div class="card-header-gradient">
            <div class="card-header-row">
                <h2 class="card-header-title"><i class="fas fa-list"></i> All Orders</h2>
                <span class="table-count">Total: {{ $orders->total() }}</span>
            </div>
        </div>

        {{-- Filter Bar --}}
        <form method="GET" action="{{ route('orders') }}" class="filter-bar">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by order #, name, email, phone...">
            ...
            <a href="{{ route('orders') }}" class="btn btn-secondary btn-sm"><i class="fas fa-redo"></i> Reset</a>
        </form>
        ...
        <div class="table-wrapper">
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><span class="badge badge-order-id">{{ $order->order_number }}</span></td>
                        <td>
                            <div style="font-weight:700;color:#0a214f;font-size:12px;">{{ $order->first_name }} {{ $order->last_name }}</div>
                            <div style="font-size:10px;color:#6b7280;">{{ $order->email }}</div>
                            <div style="font-size:10px;color:#6b7280;">{{ $order->phone }}</div>
                        </td>
                        <td style="text-align:center;">{{ $order->items->sum('quantity') }}</td>
                        <td style="font-weight:700;color:#0a214f;">₹{{ number_format($order->total, 2) }}</td>
                        <td>
                            <span class="badge pay-{{ $order->payment_status }}">{{ ucfirst($order->payment_status) }}</span><br>
                            <span style="font-size:10px;color:#6b7280;">{{ str_replace('_', ' ', ucfirst($order->payment_method)) }}</span>
                        </td>
                        <td>
                            <form action="{{ route('order.update.status', $order->id) }}" method="POST">
                                @csrf
                                <select name="order_status" onchange="this.form.submit()"
                                        class="badge status-{{ $order->order_status }}"
                        ...
                        <td style="text-align:center;">
                            <div style="font-size:11px;color:#0a214f;font-weight:600;">{{ $order->created_at->format('d M Y') }}</div>
                            <div style="font-size:10px;color:#6b7280;">{{ $order->created_at->format('h:i A') }}</div>
                        </td>
                        <td style="text-align:center;">
                            <a href="{{ route('order.view', $order->id) }}" class="btn btn-primary btn-sm">
